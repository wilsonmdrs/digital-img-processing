/* App.js */
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import React, {useEffect, useState} from 'react'
import Chart from 'react-apexcharts'
import { HistogramChart } from '../filters'
import icon from '../assets/images/acumulated-icon.png'




 
const ApexChart = ({data, title}) =>  {

  // const [Acumulated, setAcumulated] = useState(false)

  const chanel = (number) => {
    let ch = new Array(number)
    ch[0] = 0
    ch[50] = 50
    ch[100] = 100
    ch[150] = 150
    ch[200] = 200
    ch[255] = 255
    return ch
  }


    useEffect(() => {
      
      setState({...state,  series: [
        {
          name: 'Red',
          color: "red",
          data: [...data.red]
        },
        {
          name: 'Green',
          color: "green",
          data: [...data.green]
        },
        {
          name: 'Blue',
          color: "blue",
          data:  [...data.blue]
        }
      ],})
    },[data])
 
    const [state, setState] = useState({
        series: [
          {
            name: 'Red',
            color: "red",
            data: !!data ? data.red : []
          },
          {
            name: 'Green',
            color: "green",
            data: !!data ? data.green : []
          },
          {
            name: 'Blue',
            color: "blue",
            data: !!data ? data.blue : []
          }
        ],
        fill: {
          type: 'gradient',
          gradient: {
            shadeIntensity: 1,
            opacityFrom: 0.7,
            opacityTo: 0.9,
            stops: [0, 100]
          }
        },
          
        options: {
          chart: {
            width:"100%",
            height: 140,
            type: 'area',
          //   toolbar: {
          //     tools:{
          //       customIcons:[{
          //         icon:`<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="chart-line" class="svg-inline--fa fa-chart-line fa-w-16" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path fill="currentColor" d="M496 384H64V80c0-8.84-7.16-16-16-16H16C7.16 64 0 71.16 0 80v336c0 17.67 14.33 32 32 32h464c8.84 0 16-7.16 16-16v-32c0-8.84-7.16-16-16-16zM464 96H345.94c-21.38 0-32.09 25.85-16.97 40.97l32.4 32.4L288 242.75l-73.37-73.37c-12.5-12.5-32.76-12.5-45.25 0l-68.69 68.69c-6.25 6.25-6.25 16.38 0 22.63l22.62 22.62c6.25 6.25 16.38 6.25 22.63 0L192 237.25l73.37 73.37c12.5 12.5 32.76 12.5 45.25 0l96-96 32.4 32.4c15.12 15.12 40.97 4.41 40.97-16.97V112c.01-8.84-7.15-16-15.99-16z"></path></svg>`,
          //         index:4,
          //         title: "Acumulated Histogram",
          //         class: "custom-icon",
          //         click:() =>  {
          //           setAcumulated(!Acumulated)
          //           if (Acumulated === true) {
          //             setAcumulated(false)
          //           }else if (Acumulated === false) {
          //             setAcumulated(true)
          //           }
          //         }
          //       }]
          //     }
          // },
          },
          dataLabels: {
            enabled: false
          },
          stroke: {
            curve: 'smooth'
          },
          xaxis: {
            type: 'Canal',
            categories: chanel(256)
          },
          theme: {
            mode: 'dark'
          },
          title: {
            text:title
          }
        }
      }
    )


  

    return (
      <div id="chart" >
        <Chart options={state.options} series={state.series} type="area" height={200}  />
      </div>
    )
  }
 
export default ApexChart